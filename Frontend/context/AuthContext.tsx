...
function buildFallbackUser(args: {
  ...
  const derivedName = args.remoteName || args.storedName || (email.includes('@') ? email.split('@')[0] : email) || 'AR Creator';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  const { user: firebaseUser, loading, signIn: firebaseSignIn, signUp: firebaseSignUp, signOutUser, getIdToken } = useFirebaseAuth();
  ...
  const refreshProfile = useCallback(async () => {
    ...
    const [[, storedName], [, storedRole]] = await AsyncStorage.multiGet([
      PROFILE_NAME_STORAGE_KEY,
      PROFILE_ROLE_STORAGE_KEY,
    ]);

    try {
      await syncUser(nextToken, firebaseUser.email);
      const profile = await fetchProfile(nextToken);
      const resolvedName = profile.databaseUser?.name || profile.name || storedName || (firebaseUser.email ? firebaseUser.email.split('@')[0] : 'AR Creator');
      const resolvedRole = storedRole || DEFAULT_ROLE;

      await AsyncStorage.multiSet([
        [PROFILE_NAME_STORAGE_KEY, resolvedName],
        [PROFILE_ROLE_STORAGE_KEY, resolvedRole],
      ]);
  ...
  const signUp = async (payload: { name: string; email: string; password: string; role?: string }) => {
    ...
    await AsyncStorage.multiSet([
      [PROFILE_NAME_STORAGE_KEY, payload.name.trim() || 'AR Creator'],
      [PROFILE_ROLE_STORAGE_KEY, payload.role?.trim() || DEFAULT_ROLE],
    ]);
  };
  ...
  const value = useMemo(
    ...
    [bootstrapped, firebaseUser, loading, onboardingComplete, refreshProfile, token, user]
  );
