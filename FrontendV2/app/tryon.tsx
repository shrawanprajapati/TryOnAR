...
function getActiveCategoryName(selectedCategory: Category | null, initialCategory: string | undefined) {
  return selectedCategory?.name || initialCategory || 'Item';
}
...
export default function TryOnScreen() {
  ...
  const activeCategoryName = getActiveCategoryName(selectedCategory, initialCategory);
  const hostedBoothCategory = resolveHostedBoothCategory(activeCategoryName, productName);
  ...
  const openHostedBooth = async (entryPoint: 'live' | 'gallery') => {
    if (!hostedBoothCategory) {
      return false;
    }

    const viewerUrl = buildViewerUrl({
      mode: hostedBoothCategory,
      category: hostedBoothCategory,
    ...
    try {
      ...
      Alert.alert(
        'Viewer unavailable',
        error instanceof Error ? error.message : 'The hosted AR booth could not be opened right now.'
      );
      return true;
    }
  };
  ...
  const handleOpenGallery = async () => {
    if (await openHostedBooth('gallery')) {
      return;
    }
  ...
  const processWithML = async (photoUri: string) => {
    const activeCategory = getActiveCategoryName(selectedCategory, initialCategory);
          ...
          {isProcessing ? (
            ...
            <View style={styles.actionButtonsRow}>
              <TouchableOpacity
                ...
                >
                <Ionicons
                  name={hostedBoothCategory ? 'open-outline' : 'images'}
                  size={24}
                  color={canRun ? accent : theme.subText}
                  style={{ marginBottom: 5 }}
                />
                <Text style={[styles.halfButtonText, { color: theme.text }]}>
                  {hostedBoothCategory ? 'Open Booth' : 'Gallery'}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
              ...
              >
                <Ionicons
                  name={hostedBoothCategory ? 'globe-outline' : 'camera'}
                  size={24}
                  color="#fff"
                  style={{ marginBottom: 5 }}
                />
                <Text style={[styles.halfButtonText, { color: '#fff' }]}>
                  {hostedBoothCategory ? 'Hosted AR' : 'Live AR'}
                </Text>
              </TouchableOpacity>
            </View>
          )}
